"""Released random-forest method, preserved as a non-RL benchmark."""
...
def print_ns3_commands(best: pd.DataFrame, start_rng_run: int) -> None:
    print("\n=== ns-3 commands for suggested candidates (2D RF benchmark) ===")
    for offset, (_, row) in enumerate(best.iterrows()):
        rng = start_rng_run + offset
        cmd = [
            './ns3 run "uav-isac-drl',
            f'--RngRun={rng}',
        ]
        for feature in FEATURES:
...
def main() -> None:
    ...
    try:
        ...
        joblib.dump({"model": model, "features": FEATURES}, "offline_rf_model.pkl")
