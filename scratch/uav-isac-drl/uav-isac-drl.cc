...
// Synchronous newline-delimited JSON bridge used for RL training.  Unlike the
// deployment-oriented UDP bridge, each observation is paired with exactly one
// action before simulated time advances.
class SynchronousRlBridge
{
public:
  explicit SynchronousRlBridge(uint16_t port)
  {
    m_listenSock = socket(AF_INET, SOCK_STREAM, 0);
    NS_ABORT_MSG_IF(m_listenSock < 0, "Could not create RL TCP socket");
    ...
    NS_ABORT_MSG_IF(bind(m_listenSock,
                         reinterpret_cast<struct sockaddr*>(&addr),
                         sizeof(addr)) < 0,
                    "Could not bind RL TCP socket on port " << port);
    NS_ABORT_MSG_IF(listen(m_listenSock, 1) < 0, "Could not listen on RL TCP socket");

    std::cout << "[RL] Waiting for synchronous controller on 127.0.0.1:"
              << port << std::endl;
    m_clientSock = accept(m_listenSock, nullptr, nullptr);
    NS_ABORT_MSG_IF(m_clientSock < 0, "Could not accept RL controller connection");
    std::cout << "[RL] Controller connected" << std::endl;
  }

  ~SynchronousRlBridge()
  {
    if (m_clientSock >= 0) close(m_clientSock);
  ...
  bool SendLine(const std::string& line)
  {
    ...
    {
      ssize_t n = send(m_clientSock, payload.data() + sent,
  ...
  bool ReceiveAction(uint32_t expectedEpisode,
                     uint32_t expectedStep,
                     uint32_t& action)
  {
    std::string line;
    while (true)
    {
      std::size_t newline = m_receiveBuffer.find('\n');
      if (newline != std::string::npos)
      {
        line = m_receiveBuffer.substr(0, newline);
        m_receiveBuffer.erase(0, newline + 1);
        break;
      }

      char buf[1024];
      ssize_t n = recv(m_clientSock, buf, sizeof(buf), 0);
      if (n <= 0) return false;
      m_receiveBuffer.append(buf, static_cast<std::size_t>(n));
    }

    auto extractUnsigned = [&](const std::string& key, uint32_t fallback) {
      ...
      try
      {
        return static_cast<uint32_t>(std::stoul(line.substr(pos, end - pos)));
      }
      catch (...)
      {
        return fallback;
    ...
    uint32_t episode = extractUnsigned("episode_id", UINT32_MAX);
    uint32_t step = extractUnsigned("step_id", UINT32_MAX);
    action = extractUnsigned("action", UINT32_MAX);
    if (episode != expectedEpisode || step != expectedStep || action >= 25)
    {
      std::cerr << "[RL] Rejected mismatched/invalid action: " << line << std::endl;
      return false;
    }
    return true;
  }
...
public:
  ...
  void SetTargetSpeed (double ms) { SetMeanSpeedValue(ms); }
  double GetTargetSpeed () const { return m_meanSpeed; }
...
private:
  ...
  void SenseAndSend()
  {
    ...
    for (auto& cow : *m_cows)
    {
      ...
      // Cow positions are on the ground (z=0).  Using u.z for both endpoints
      // silently removed altitude from the radar range and made altitude
      // actions physically irrelevant to sensing.
      double R = CalculateDistance(u, cpos);
...
bool        enableRl   = false;
uint16_t    rlPort     = 5557;
uint32_t    episodeId  = 0;
double      controlInterval = 10.0;
double      batteryBudgetJ = 100000.0;
double      energyRewardWeight = 1.0;
std::string summaryFile = "run_summary.csv";
std::string kpiFile = "control_kpi_log.csv";
cmd.AddValue("enableXapp", "Enable xApp UDP bridge (0/1)", enableXapp);
cmd.AddValue("xappHost",   "xApp host IP",                 xappHost);
cmd.AddValue("enableRl", "Enable synchronous TCP RL environment (0/1)", enableRl);
cmd.AddValue("rlPort", "Synchronous RL TCP port", rlPort);
cmd.AddValue("episodeId", "RL episode identifier", episodeId);
cmd.AddValue("controlInterval", "RL/xApp decision interval (s)", controlInterval);
cmd.AddValue("batteryBudgetJ", "Per-UAV battery budget (J)", batteryBudgetJ);
cmd.AddValue("energyRewardWeight", "Energy term weight in RL reward", energyRewardWeight);
cmd.AddValue("summaryFile", "Per-run summary CSV path", summaryFile);
cmd.AddValue("kpiFile", "Window KPI CSV path", kpiFile);
// ─────────────────────────────────────────────────────────

cmd.Parse(argc, argv);
NS_ABORT_MSG_IF(enableRl && enableXapp,
                "Use either synchronous RL mode or asynchronous xApp mode, not both");
NS_ABORT_MSG_IF(controlInterval <= 0.0, "controlInterval must be positive");
...
initialPosition->Add(Vector(240, 240, uav3z));
  ...
  std::array<Ptr<EnhancedGaussMarkovMobilityModel>, 4> mobilityModels;
  for (uint32_t i = 0; i < mobilityModels.size(); ++i)
  {
    mobilityModels[i] = DynamicCast<EnhancedGaussMarkovMobilityModel>(
        uavNodes.Get(i)->GetObject<MobilityModel>());
    NS_ABORT_MSG_IF(!mobilityModels[i],
                    "UAV " << i << " does not use EnhancedGaussMarkovMobilityModel");
  }
...
Ptr<Ipv4FlowClassifier> classifier =
  ...
  // The UDP bridge is retained for deployment emulation.  RL training uses a
  // synchronous TCP bridge so every action is paired with one KPI window.
  ...
  std::unique_ptr<SynchronousRlBridge> rlBridge;
  if (enableRl)
  {
    rlBridge = std::make_unique<SynchronousRlBridge>(rlPort);
  }

  // ============================================================
  // KPI tracking CSV — records metrics every 10 sim-seconds
  // ============================================================
  std::ofstream xappKpiLog(kpiFile);
   xappKpiLog
    << "sim_time_s,"
    // System-level KPIs
    ...
    // Active parameters
    << "survSenseDt,patSenseDt,rapidSenseDt,stratSenseDt,"
    << "survTargetSpeed,patTargetSpeed,rapidTargetSpeed,stratTargetSpeed,"
    << "survTargetAltitude,patTargetAltitude,rapidTargetAltitude,stratTargetAltitude,"
    << "survDt,patDt,rapidDt,stratDt\n";

  std::array<double*, 4> sensingInterval = {
    &survSenseDt, &patSenseDt, &rapidSenseDt, &stratSenseDt
  };
  const std::array<double, 4> speedLower = {6.0, 10.0, 18.0, 8.0};
  const std::array<double, 4> speedUpper = {25.0, 35.0, 40.0, 25.0};
  const std::array<double, 4> altitudeLower = {survZmin, patZmin, rapidZmin, stratZmin};
  const std::array<double, 4> altitudeUpper = {survZmax, patZmax, rapidZmax, stratZmax};

  auto applyDiscreteAction = [&](uint32_t action) {
    if (action == 0) return;
    uint32_t role = (action - 1) / 6;
    uint32_t operation = (action - 1) % 6;
    NS_ABORT_MSG_IF(role >= numUAVs, "Invalid RL role in action " << action);
    double oldSense = *sensingInterval[role];
    double oldSpeed = mobilityModels[role]->GetTargetSpeed();
    double oldAltitude = mobilityModels[role]->GetTargetAltitude();

    if (operation == 0 || operation == 1)
    {
      double delta = (operation == 0) ? -0.1 : 0.1;
      *sensingInterval[role] = ClampToRange(*sensingInterval[role] + delta, 0.3, 3.0);
      sensingApps[role]->SetSensingInterval(*sensingInterval[role]);
    }
    else if (operation == 2 || operation == 3)
    {
      double delta = (operation == 2) ? -1.0 : 1.0;
      double target = ClampToRange(mobilityModels[role]->GetTargetSpeed() + delta,
                                   speedLower[role], speedUpper[role]);
      mobilityModels[role]->SetTargetSpeed(target);
    }
    else
    {
      double delta = (operation == 4) ? -5.0 : 5.0;
      double target = ClampToRange(mobilityModels[role]->GetTargetAltitude() + delta,
                                   altitudeLower[role], altitudeUpper[role]);
      mobilityModels[role]->SetTargetAltitude(target);
    }
    std::cout << "[RL] action=" << action << " role=" << role
              << " sensing=" << oldSense << "->" << *sensingInterval[role]
              << " speed=" << oldSpeed << "->" << mobilityModels[role]->GetTargetSpeed()
              << " altitude=" << oldAltitude << "->"
              << mobilityModels[role]->GetTargetAltitude() << std::endl;
  };
  ...
  controlCycle = [&]() {
    if (enableXapp || enableRl)
    {
      ...
      for (const auto& kv : flowStats)
      {
        ...
        const FlowSnapshot previous = previousFlowStats[kv.first];
        uint64_t deltaRxBytes = st.rxBytes - previous.rxBytes;
        uint32_t deltaRxPackets = st.rxPackets - previous.rxPackets;
        uint32_t deltaTxPackets = st.txPackets - previous.txPackets;
        uint32_t deltaLostPackets = st.lostPackets - previous.lostPackets;
        double deltaDelay = st.delaySum.GetSeconds() - previous.delaySeconds;

        windowThr += deltaRxBytes * 8.0 / controlInterval / 1e6;
        if (deltaRxPackets > 0)
          windowDelay += deltaDelay / deltaRxPackets * 1000.0;
        if (deltaTxPackets > 0)
          windowLoss += 100.0 * deltaLostPackets / deltaTxPackets;

        previousFlowStats[kv.first] = {
          st.rxBytes, st.rxPackets, st.txPackets, st.lostPackets,
      ...
      double deltaEnergy = std::max(0.0, totalEnergy - previousTotalEnergy);
      double stepEnergyBudget = numUAVs * batteryBudgetJ *
                                controlInterval / simulationTime;
      double energyPenalty = energyRewardWeight * deltaEnergy /
                             std::max(stepEnergyBudget, 1e-9);
      double reward = jscore - energyPenalty;
      previousTotalEnergy = totalEnergy;

      bool batteryTerminated = false;
      for (uint32_t i = 0; i < numUAVs; ++i)
        batteryTerminated = batteryTerminated || (uavEtot[i] >= batteryBudgetJ);
      bool timeTruncated = simNow + 1e-9 >= simulationTime;
      ...
      xappKpiLog
        ...
        << mobilityModels[0]->GetTargetSpeed() << ","
        << mobilityModels[1]->GetTargetSpeed() << ","
        << mobilityModels[2]->GetTargetSpeed() << ","
        << mobilityModels[3]->GetTargetSpeed() << ","
        << mobilityModels[0]->GetTargetAltitude() << ","
        << mobilityModels[1]->GetTargetAltitude() << ","
        << mobilityModels[2]->GetTargetAltitude() << ","
        << mobilityModels[3]->GetTargetAltitude() << ","
        << survDt  << "," << patDt   << ","
        << rapidDt << "," << stratDt << "\n";
      xappKpiLog.flush();
      ...
      if (enableXapp)
      {
        ...
        if (kpiReporter->ReceiveParamUpdate(newParams))
        {
          for (uint32_t i = 0; i < numUAVs; ++i)
          {
            const std::array<std::string, 4> senseKeys = {
              "survSenseDt", "patSenseDt", "rapidSenseDt", "stratSenseDt"
            };
            const std::array<std::string, 4> speedKeys = {
              "survVmax", "patVmax", "rapidVmax", "stratVmax"
            };
            const std::array<std::string, 4> zMinKeys = {
              "survZmin", "patZmin", "rapidZmin", "stratZmin"
            };
            const std::array<std::string, 4> zMaxKeys = {
              "survZmax", "patZmax", "rapidZmax", "stratZmax"
            };
            if (newParams.count(senseKeys[i]))
            {
              *sensingInterval[i] = ClampToRange(newParams[senseKeys[i]], 0.3, 3.0);
              sensingApps[i]->SetSensingInterval(*sensingInterval[i]);
            }
            if (newParams.count(speedKeys[i]))
              mobilityModels[i]->SetTargetSpeed(ClampToRange(
                  newParams[speedKeys[i]], speedLower[i], speedUpper[i]));
            if (newParams.count(zMinKeys[i]) || newParams.count(zMaxKeys[i]))
            {
              double lo = newParams.count(zMinKeys[i]) ? newParams[zMinKeys[i]]
                                                       : altitudeLower[i];
              double hi = newParams.count(zMaxKeys[i]) ? newParams[zMaxKeys[i]]
                                                       : altitudeUpper[i];
              mobilityModels[i]->SetTargetAltitude((lo + hi) / 2.0);
            }
          }
        }
      }
      ...
      if (enableRl)
      {
        double cowMeanX = 0.0, cowMeanY = 0.0;
        for (const auto& cow : cows)
        {
          cowMeanX += cow.pos.x;
          cowMeanY += cow.pos.y;
        }
        cowMeanX /= std::max<std::size_t>(1, cows.size());
        cowMeanY /= std::max<std::size_t>(1, cows.size());
        double cowVarX = 0.0, cowVarY = 0.0;
        for (const auto& cow : cows)
        {
          cowVarX += (cow.pos.x - cowMeanX) * (cow.pos.x - cowMeanX);
          cowVarY += (cow.pos.y - cowMeanY) * (cow.pos.y - cowMeanY);
        }
        cowVarX /= std::max<std::size_t>(1, cows.size());
        cowVarY /= std::max<std::size_t>(1, cows.size());

        std::vector<double> observation;
        observation.reserve(50);
        observation.insert(observation.end(), {
          Pdet_n, RMSE_n, Thr_n, Delay_n, Loss_n
        });
        for (uint32_t i = 0; i < numUAVs; ++i)
          observation.push_back(ClampToRange(1.0 - uavEtot[i] / batteryBudgetJ, 0.0, 1.0));
        for (uint32_t i = 0; i < numUAVs; ++i)
        {
          Vector position = mobilityModels[i]->GetPosition();
          Vector velocity = mobilityModels[i]->GetVelocity();
          observation.insert(observation.end(), {
            ClampToRange(position.x / (areaSize / 2.0), -1.0, 1.0),
            ClampToRange(position.y / (areaSize / 2.0), -1.0, 1.0),
            ClampToRange(position.z / 150.0, 0.0, 1.0),
            ClampToRange(velocity.x / 45.0, -1.0, 1.0),
            ClampToRange(velocity.y / 45.0, -1.0, 1.0),
            ClampToRange(velocity.z / 2.0, -1.0, 1.0)
          });
        }
        observation.insert(observation.end(), {
          ClampToRange(cowMeanX / (areaSize / 2.0), -1.0, 1.0),
          ClampToRange(cowMeanY / (areaSize / 2.0), -1.0, 1.0),
          ClampToRange(std::sqrt(cowVarX) / (areaSize / 2.0), 0.0, 1.0),
          ClampToRange(std::sqrt(cowVarY) / (areaSize / 2.0), 0.0, 1.0)
        });
        for (uint32_t i = 0; i < numUAVs; ++i)
          observation.push_back((*sensingInterval[i] - 0.3) / 2.7);
        for (uint32_t i = 0; i < numUAVs; ++i)
          observation.push_back(mobilityModels[i]->GetTargetSpeed() / 45.0);
        for (uint32_t i = 0; i < numUAVs; ++i)
          observation.push_back(mobilityModels[i]->GetTargetAltitude() / 150.0);
        observation.push_back(ClampToRange(simNow / simulationTime, 0.0, 1.0));
        NS_ABORT_MSG_IF(observation.size() != 50,
                        "RL observation must contain exactly 50 values");

        std::ostringstream state;
        state.precision(10);
        state << "{\"type\":\"transition\",\"episode_id\":" << episodeId
              << ",\"step_id\":" << rlStep
              << ",\"sim_time\":" << simNow
              << ",\"reward\":" << reward
              << ",\"terminated\":" << (batteryTerminated ? "true" : "false")
              << ",\"truncated\":" << (timeTruncated ? "true" : "false")
              << ",\"pdet\":" << pdet
              << ",\"rmse_m\":" << rmse
              << ",\"throughput_mbps\":" << avgThr
              << ",\"delay_ms\":" << avgDelay
              << ",\"loss_pct\":" << avgLoss
        ...
        NS_ABORT_MSG_IF(!rlBridge->SendLine(state.str()),
                        "RL controller disconnected while receiving state");
        if (!batteryTerminated && !timeTruncated)
        {
          uint32_t action = 0;
          NS_ABORT_MSG_IF(!rlBridge->ReceiveAction(episodeId, rlStep, action),
    ...
    if (next <= simulationTime + 1e-9)
      Simulator::Schedule(Seconds(controlInterval), controlCycle);
  };

  if (enableXapp || enableRl)
    Simulator::Schedule(Seconds(enableRl ? controlInterval : 20.0), controlCycle);

  Simulator::Stop(Seconds(simulationTime + (enableRl ? 1e-6 : 0.0)));
  Simulator::Run();
...
// append summary row
// append summary row
bool needHeader = FileEmptyOrMissing(summaryFile);
std::ofstream runSummary(summaryFile, std::ios::app);
