...
#include "oran-lm-nr-2-nr-torch-handover.h"
...
NS_LOG_COMPONENT_DEFINE("OranLmNr2NrTorchHandover");
NS_OBJECT_ENSURE_REGISTERED(OranLmNr2NrTorchHandover);

TypeId
OranLmNr2NrTorchHandover::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::OranLmNr2NrTorchHandover")
            .SetParent<OranLm>()
            .AddConstructor<OranLmNr2NrTorchHandover>()
            .AddAttribute("TorchModelPath",
                          "The file path of the ML model.",
                          StringValue("saved_trained_classification_pytorch.pt"),
                          MakeStringAccessor(&OranLmNr2NrTorchHandover::SetTorchModelPath),
                          MakeStringChecker());

    return tid;
}

OranLmNr2NrTorchHandover::OranLmNr2NrTorchHandover()
{
    NS_LOG_FUNCTION(this);

    m_name = "OranLmNr2NrTorchHandover";
}
...
void
OranLmNr2NrTorchHandover::SetTorchModelPath(const std::string& torchModelPath)
{
...
std::vector<OranLmNr2NrTorchHandover::UeInfo>
OranLmNr2NrTorchHandover::GetUeInfos(Ptr<OranDataRepository> data) const
{
    NS_LOG_FUNCTION(this << data);

    std::vector<UeInfo> ueInfos;
    for (const auto ueId : data->GetNrUeE2NodeIds())
    {
        UeInfo ueInfo;
        ueInfo.nodeId = ueId;
        bool found;
...
std::vector<OranLmNr2NrTorchHandover::GnbInfo>
OranLmNr2NrTorchHandover::GetGnbInfos(Ptr<OranDataRepository> data) const
{
    NS_LOG_FUNCTION(this << data);

    std::vector<GnbInfo> gnbInfos;
    for (const auto gnbId : data->GetNrGnbE2NodeIds())
    {
        GnbInfo gnbInfo;
        ...
        if (found)
        {
            ...
            {
                NS_LOG_INFO("Could not find NR eNB location for E2 Node ID = " << gnbInfo.nodeId);
            }
        }
        else
        {
            NS_LOG_INFO("Could not find NR eNB cell info for E2 Node ID = " << gnbInfo.nodeId);
        }
    }
    return gnbInfos;
}
...
std::vector<Ptr<OranCommand>>
OranLmNr2NrTorchHandover::GetHandoverCommands(
    Ptr<OranDataRepository> data,
    std::vector<OranLmNr2NrTorchHandover::UeInfo> ueInfos,
    std::vector<OranLmNr2NrTorchHandover::GnbInfo> gnbInfos)
{
    NS_LOG_FUNCTION(this << data);
