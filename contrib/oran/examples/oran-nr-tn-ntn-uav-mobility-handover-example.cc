 ...
 *
 * \code{.unparsed}
 * ./ns3 run "oran-nr-tn-ntn-uav-mobility-handover-example --PrintHelp"
 * \endcode
 *
...
// Variables
uint32_t numGroundUesS1 = 100; // UES1 
uint32_t numGroundUesS2 = 100;   // UES2
                  ...
                  uint8_t componentCarrierId)
{
    ...
    const char* type =
        g_ntnCellIds.count(cellId) ? "NTN" : "TN";
...
main(int argc, char* argv[])
{
    ...
    */
    ...
    for (uint32_t i = 0; i < groundUeNodesS1.GetN(); ++i)
    {
        ...
        // QoS flow mapping → BWPs 
        // DL XR video → BWP1 (FDD-DL)
    ...
    for (NetDeviceContainer::Iterator it = groundNrDevsS2.Begin(); it != groundNrDevsS2.End(); ++it)
    {
        ...
        if (nrUeDevice)
        {
            ...
            uePhy->TraceConnectWithoutContext(
                "ReportUeMeasurements",
                MakeBoundCallback(&TraceRsrpRsrqSinr,
