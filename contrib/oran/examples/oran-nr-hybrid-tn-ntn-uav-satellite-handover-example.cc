...
NS_LOG_COMPONENT_DEFINE("OranNrHybridTnNtnUavSatelliteHandoverExample");
 ...
 *
 * \code{.unparsed}
 * ./ns3 run "oran-nr-hybrid-tn-ntn-uav-satellite-handover-example --PrintHelp"
 * \endcode
 *
...
static std::string g_uavPredictorOnnxPath =
    "uav_underserved_heatmap_gru_ir8.onnx";
/////
const static float TN_GNB_HEIGHT = 25;


// Variables
uint32_t numGroundUesS1 = 30; // UES1 
uint32_t numGroundUesS2 = 30; // UES2

uint32_t numTnGnbs = 5;
uint32_t numNtnGnbs = 4; // e.g., uav gnbs for ntn area
...
struct QosUeRecord
{
    std::string role;     // "UES1" or "UES2"
    uint32_t localId = 0;
    uint64_t imsi = 0;
    Ipv4Address ip;

    Ptr<NrUeNetDevice> ueDev;   // ADD THIS

    QosIntervalMetrics dl;
    QosIntervalMetrics ul;
};

static std::vector<QosUeRecord> g_qosUes;
static std::unordered_map<uint32_t, size_t> g_ipToQosIndex;

static uint16_t
GetServingCellId(const QosUeRecord& ue)
{
    if (!ue.ueDev || !ue.ueDev->GetRrc())
    {
        return 0;
    }
    return ue.ueDev->GetRrc()->GetCellId();
}

static std::string
GetServingCellType(uint16_t cellId)
{
    if (cellId == 0)
    {
        return "NONE";
    ...
    return g_ntnCellIds.count(cellId) ? "NTN" : "TN";
}
...
static void
RegisterQosUe(const std::string& role,
              uint32_t localId,
              uint64_t imsi,
              Ipv4Address ip,
              Ptr<NrUeNetDevice> ueDev)
{
    ...
    size_t idx = g_qosUes.size();
    g_qosUes.push_back(rec);
    g_ipToQosIndex[ip.Get()] = idx;
}

static int
GetQosIndexFromIpv4(Ipv4Address ip)
{
    auto it = g_ipToQosIndex.find(ip.Get());
    if (it == g_ipToQosIndex.end())
    {
        return -1;
    }
...
ThroughputMonitor(FlowMonitorHelper* fmhelper, Ptr<FlowMonitor> flowMon)
{
    ...
    std::vector<TmpAgg> dlAgg(g_qosUes.size());
    std::vector<TmpAgg> ulAgg(g_qosUes.size());

    auto flowStats = flowMon->GetFlowStats();

    auto ueNetwork = Ipv4Address("7.0.0.0");
    auto ueNetworkMask = Ipv4Mask("255.0.0.0");
    ...
    for (const auto& stats : flowStats)
    {
        ...
        bool isDl = ueNetworkMask.IsMatch(ueNetwork, fiveTuple.destinationAddress);
        bool isUl = ueNetworkMask.IsMatch(ueNetwork, fiveTuple.sourceAddress);
        ...
        Ipv4Address ueIp = isDl ? fiveTuple.destinationAddress
                                : fiveTuple.sourceAddress;

        int qosIdx = GetQosIndexFromIpv4(ueIp);
        if (qosIdx < 0)
        {
            continue;
        }

        auto prevIt = g_prevFlowStats.find(flowId);

        if (prevIt == g_prevFlowStats.end())
        {
            FlowIntervalSnapshot snap;
            snap.txPackets = st.txPackets;
            snap.rxPackets = st.rxPackets;
            snap.rxBytes   = st.rxBytes;
            snap.delaySum  = st.delaySum;
            snap.jitterSum = st.jitterSum;

            g_prevFlowStats[flowId] = snap;
            continue;
        }

        FlowIntervalSnapshot& prev = prevIt->second;
        ...
        TmpAgg& agg = isDl ? dlAgg[qosIdx] : ulAgg[qosIdx];

        agg.txPackets += dTxPackets;
        agg.rxPackets += dRxPackets;
        agg.rxBytes   += dRxBytes;
        agg.delaySum  += dDelaySum;
        agg.jitterSum += dJitterSum;
    }

    auto ConvertAggToMetrics = [&](const TmpAgg& agg) -> QosIntervalMetrics
    {
        QosIntervalMetrics m;

        m.txPackets = agg.txPackets;
        m.rxPackets = agg.rxPackets;
        m.rxBytes = agg.rxBytes;

        m.pdr = (agg.txPackets > 0)
                    ? (100.0 * static_cast<double>(agg.rxPackets) /
                       static_cast<double>(agg.txPackets))
                    : std::numeric_limits<double>::quiet_NaN();

        m.delay = (agg.rxPackets > 0)
                    ? (agg.delaySum.GetSeconds() /
                       static_cast<double>(agg.rxPackets))
                    : std::numeric_limits<double>::quiet_NaN();

        m.jitter = (agg.rxPackets > 0)
                    ? (agg.jitterSum.GetSeconds() /
                       static_cast<double>(agg.rxPackets))
                    : std::numeric_limits<double>::quiet_NaN();

        m.throughput = (intervalSec > 0.0)
    ...
    for (size_t i = 0; i < g_qosUes.size(); ++i)
    {
        g_qosUes[i].dl = ConvertAggToMetrics(dlAgg[i]);
        g_qosUes[i].ul = ConvertAggToMetrics(ulAgg[i]);
    }

    std::ofstream qosVsTime;
    qosVsTime.open(ns3_dir + "qos-vs-time.txt",
                   std::ofstream::out | std::ofstream::app);

    double t = Simulator::Now().GetSeconds();

    for (const auto& ue : g_qosUes)
    {
        uint16_t servingCell = GetServingCellId(ue);
        std::string servingType = GetServingCellType(servingCell);
                      ...
                      const std::string& filename)
{
    ...
    for (const auto& kv : stats)
    {
        ...
        double thrMbps = (txDuration > 0)
            ? (st.rxBytes * 8.0 / txDuration / 1e6)
            : 0.0;
        double pdr = (st.txPackets > 0)
            ? (100.0 * static_cast<double>(st.rxPackets) / static_cast<double>(st.txPackets))
            : 0.0;

        out << "  AvgThroughput: " << thrMbps << " Mbps\n";
        out << "  PDR: " << pdr << " %\n";
...
main(int argc, char* argv[])
{
    ...
    bool enablePositionTrace = false;
    ...
    double s1TrafficStart = 8.0;
    double s2TrafficStartAfterAttach = 3.0;
    double initMinRsrpDbm = -140.0;
    double initRetryIntervalSec = 1.0;
    ...
    cmd.AddValue("s1-traffic-start", "UES1 traffic start time in seconds", s1TrafficStart);
    cmd.AddValue("s2-traffic-start-after-attach", "UES2 traffic start delay after late attach in seconds", s2TrafficStartAfterAttach);
    ...
    */
    ...
    for (uint32_t i = 0; i < groundNrDevsS1.GetN(); ++i)
    {
        ...
        RegisterQosUe("UES1", i, ue->GetImsi(), ip, ue);
    }
    ...
    for (uint32_t i = 0; i < groundNrDevsS2.GetN(); ++i)
    {
        ...
        RegisterQosUe("UES2", i, ue->GetImsi(), ip, ue);
    }
    ...
    // Start/stop times
    xrDlSinks.Start(Seconds(std::max(0.0, s1TrafficStart - 1.0)));
    xrDlSinks.Stop(simTime + Seconds(15));

    xrDlSenders.Start(Seconds(s1TrafficStart));
    xrDlSenders.Stop(simTime + Seconds(10));

    xrUlSinks.Start(Seconds(std::max(0.0, s1TrafficStart - 1.0)));
    xrUlSinks.Stop(simTime + Seconds(15));

    xrUlSenders.Start(Seconds(s1TrafficStart + 1.0));
    ...
    for (uint16_t i = 0; i < groundUeNodesS2.GetN(); i++)
    {
        ...
        gdl.direction      = NrQosRule::DOWNLINK;
        voiceRule->Add(gdl);
    ...
    ueAppsS2.Start(Seconds(std::max(0.0, groundAttachDelay + s2TrafficStartAfterAttach - 1.0)));
    groundRemoteAppsS2.Start(tLateAttach + Seconds(s2TrafficStartAfterAttach));
    ...
    if (enableFlowMonitor)
    {
        ...
        qos_vs_time << "Time,Role,LocalUE,IMSI,IP,ServingCell,ServingType,Dir,Delay,Jitter,Throughput,PDR,TxPackets,RxPackets,RxBytes"
            << std::endl;
