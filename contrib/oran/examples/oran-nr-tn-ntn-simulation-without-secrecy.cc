...
int main(int argc, char* argv[])
{
        ...
        for (uint32_t idx = 0; idx < ueNodes.GetN(); idx++)
        {
            ...
            for (uint32_t netDevIdx = 0; netDevIdx < ueNodes.Get(idx)->GetNDevices(); netDevIdx++)
            {
                ...
                if (nrUeDevice)
                {
                    ...
                    // OPTIONAL (SINR): If want ORAN to see SINR, implement a reporter for DlDataSinr
    ...
    for (uint32_t i = 0; i < ueNrDevs.GetN(); ++i)
    {
        ...
        // OPTIONAL: also connect control SINR (if you want)
        phy->TraceConnectWithoutContext("DlCtrlSinr",
            MakeBoundCallback(&UeDlCtrlSinrCb, i, ueSinrStream));

        // OPTIONAL (your note): ReportRsrpSinrTrace() exists as a *method*; it fires a "RSRP+SINR" trace in some setups.
        // If your build exposes a trace source (name depends on version), connect it here (commented to avoid compile issues):
        // phy->TraceConnectWithoutContext("RsrpSinr", ... );
    }
