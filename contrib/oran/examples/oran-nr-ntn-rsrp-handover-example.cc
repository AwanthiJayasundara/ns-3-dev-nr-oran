...
NS_LOG_COMPONENT_DEFINE("OranNrNtnRsrpHandoverExample");
 ...
 *
 * \code{.unparsed}
 * ./ns3 run "oran-nr-ntn-rsrp-handover-example --PrintHelp"
 * \endcode
 *
...
static std::string s_trafficTraceFile = "results/nr/ntn/nr-traffic-trace.tr";
static std::string s_positionTraceFile = "results/nr/ntn/nr-position-trace.tr";
static std::string s_handoverTraceFile = "results/nr/ntn/nr-handover-trace.tr";
static std::string ns3_dir = "results/nr/ntn/";
...
ThroughputMonitor(FlowMonitorHelper* fmhelper, Ptr<FlowMonitor> flowMon)
{
    ...
    qos_vs_time.open("results/nr/ntn/qos-vs-time.txt", std::ofstream::out | std::ofstream::app);
...
install_mobility(NodeContainer staticNodes, NodeContainer gnbNodes, NodeContainer ueNodes)
{
    ...
    // UAV-gNB initial placement (random in a box) and mobility model 
    Ptr<RandomBoxPositionAllocator> gnbBox = CreateObject<RandomBoxPositionAllocator>();
    gnbBox->SetAttribute("X", StringValue("ns3::UniformRandomVariable[Min=-1990|Max=1990]"));
    gnbBox->SetAttribute("Y", StringValue("ns3::UniformRandomVariable[Min=-1990|Max=1990]"));
    gnbBox->SetAttribute("Z", StringValue("ns3::UniformRandomVariable[Min=100.0|Max=200.0]"));
    ...
    gnbHelper.SetMobilityModel("ns3::RandomDirection2dMobilityModel",
                            ...
                            "Pause",  StringValue("ns3::UniformRandomVariable[Min=0.5|Max=2.0]"));
    ...
    box->SetAttribute("X", StringValue("ns3::UniformRandomVariable[Min=-1990|Max=1990]")); // initial placement region
    box->SetAttribute("Y", StringValue("ns3::UniformRandomVariable[Min=-1990|Max=1990]")); // initial placement region
    ...
    ueHelper.SetMobilityModel("ns3::RandomDirection2dMobilityModel",
                              "Bounds",
                              StringValue("-2000|2000|-2000|2000"),
                              "Speed",
...
main(int argc, char* argv[])
{
    ...
    std::string dbFileName = "ntn-nr-oran-repository.db";
    ...
    std::ofstream qos_vs_time;
    qos_vs_time.open("results/nr/ntn/qos-vs-time.txt", std::ofstream::out | std::ofstream::trunc);
