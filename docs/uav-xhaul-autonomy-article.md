...
Future 5G-Advanced and 6G systems are expected to support connectivity across heterogeneous three-dimensional network environments. Terrestrial infrastructure alone is often insufficient in remote regions, temporary crowded events, and disaster scenarios where ground base stations may be overloaded, damaged, or partially disconnected. In such cases, gNB-mounted UAVs can provide flexible aerial coverage, while satellite links can provide additional resilience when terrestrial backhaul is degraded.
...
The UAV-to-TN donor/backhaul condition is estimated using a log-distance path-loss proxy with stochastic shadowing/fading variation. This proxy is used only for the switching xApp's donor-health decision; it is separate from the 3GPP UMa, NTN-Urban, and NTN-Suburban channel models used for the NR access and satellite-backhaul links. Using the notation adopted in the system model, the donor-backhaul path loss is:

```text
L = L0 + 10 gamma log10(d / d0) + X_sh(t) + X_f(t)
```

where `d` is the distance between the UAV gNB and the selected terrestrial donor gNB, `d0` is the reference distance, `L0` is the path loss at `d0`, and `gamma` is the donor-link path-loss exponent. The stochastic terms `X_sh(t)` and `X_f(t)` represent slow shadowing and fading-related variation in dB. The estimated donor-backhaul RSRP used by the xApp is:

```text
P_hat_RSRP_xH(d,t) = P_ref_donor - L
```
...
usable donor threshold = -100 dBm
```

The exponent `gamma = 2` corresponds to free-space-like distance loss, while larger values represent stronger attenuation in obstructed urban or non-line-of-sight donor conditions. Therefore, `gamma = 5.6` is used as a stressed obstructed-donor setting that can make UAV mission movement visible within a 120 s simulation: as the UAV moves toward underserved UEs and farther from TN donor gNBs, the estimated donor RSRP can fall below the usable threshold. The switching xApp then treats the TN donor path as unreachable and selects either `SATELLITE_FALLBACK` or `NO_BACKHAUL_AVAILABLE` depending on satellite health.
...
This figure shows the free-space and log-distance path-loss curves, the random channel variation, and the resulting estimated donor-backhaul RSRP crossing the `-100 dBm` switching threshold.
...
This scenario adds gNB-mounted UAV nodes.
...
| Healthy TN + UAV xHaul condition | Best donor RSRP >= -100 dBm | UAVs are expected to remain connected through TN donor backhaul. |
...
| UAV initial placement half-width, donor-stress runs | 3 km | UAVs initially start inside a 6 km-wide region near the TN deployment, so TN donor backhaul is likely available at the beginning. |
| UAV initial placement half-height, donor-stress runs | 1.5 km | UAVs initially start inside a 3 km-high region near the TN deployment, supporting the initial healthy-backhaul period. |
| UE mobility half-width, donor-stress runs | 10 km | UEs are distributed over a wider 20 km x 10 km emergency/service area, creating realistic weak-coverage users outside the central TN region. |
| UE mobility half-height, donor-stress runs | 5 km | Wider north/south UE placement produces underserved UE clusters that can attract UAVs away from the TN donor region. |
| UAV mission mobility half-width, donor-stress runs | 10 km | During the mission, UAVs may move across the same 20 km-wide service region as the UEs. |
| UAV mission mobility half-height, donor-stress runs | 5 km | The UAV mission area matches the UE service area, so UAV movement is tied to real underserved UE locations rather than artificial overshoot. |
| UAV mission target scale, donor-stress runs | 1 | UAVs fly directly toward underserved UE clusters; donor-backhaul stress comes from wider UE placement and UAV movement rather than artificial target expansion. |
| UAV mission speed, donor-stress runs | 25 m/s | Realistic UAV movement speed during mission repositioning. |
| UAV donor-backhaul stress method, donor-stress runs | Donor RSRP from distance, log-distance path loss, and channel variation | Degradation is created by UAV geometry and channel behavior, not by a fixed outage window or RSRP penalty. |
...
| UAV-to-TN donor path-loss exponent, donor-stress runs | 5.6 | Stressed obstructed log-distance donor-backhaul exponent used to make distance-based donor weakening visible within 120 s without a forced outage flag. |
...
| xHaul usable RSRP threshold, donor-stress runs | -100 dBm | Policy threshold above which the UAV donor path is treated as healthy; below it the donor path is treated as unreachable. |
...
| UAV-to-TN donor usable/unreachable threshold | Proposed xApp policy | 3GPP specifies RSRP measurement/reporting behavior and channel models, but it does not define a universal "healthy UAV donor backhaul" RSRP threshold. The `-100 dBm` cutoff is therefore treated as a conservative RIC policy parameter for reliable UAV donor-backhaul operation. |
...
./ns3 run "oran-nr-uav-xhaul-autonomy-example --deployment-mode=tn-uav --run-label=distance-loss-no-sat --sim-time=120 --num-uess1=50 --num-ground-ues=50 --ground-attach-delay=5 --num-tn-gnbs=4 --num-ntn-gnbs=3 --max-ues-tn=20 --max-ues-ntn=10 --rlc-mode=am --monitored-traffic=udp --monitored-dl-rate-mbps=0.2 --monitored-ul-rate-mbps=0.05 --monitored-packet-size=1000 --tn-tx-power-dbm=46 --xhaul-tx-power-dbm=46 --uav-tx-power-dbm=37 --ue-tx-power-dbm=23 --init-min-rsrp=-110 --handover-min-rsrp-dbm=-110 --enable-harq-retx=0 --pdcp-discard-timer-ms=1000 --enable-flow-monitor=1 --enable-rsrp-trace=1 --enable-position-trace=1 --enable-handover-trace=1 --enable-handover-failure-trace=1 --enable-decision-csv=1 --enable-oran-info-log=1 --uav-control-start=5 --uav-control-period=2 --uav-underserved-rsrp-thresh-dbm=-110 --ue-area-half-w-m=10000 --ue-area-half-h-m=5000 --uav-initial-area-half-w-m=3000 --uav-initial-area-half-h-m=1500 --uav-area-half-w-m=10000 --uav-area-half-h-m=5000 --uav-mission-target-scale=1 --uav-speed-mps=25 --xhaul-pathloss-exponent=5.6 --xhaul-reference-distance-m=100 --xhaul-healthy-rsrp-dbm=-100 --xhaul-switch-to-sat-ttt-s=5 --xhaul-switch-to-tn-ttt-s=5 --enable-xhaul-channel-variation=1 --xhaul-shadowing-stddev-db=6 --xhaul-fading-stddev-db=4 --channel-update-ms=100 --channel-condition-update-ms=200"

./ns3 run "oran-nr-uav-xhaul-autonomy-example --deployment-mode=tn-uav-satellite --run-label=distance-loss-sat --sim-time=120 --num-uess1=50 --num-ground-ues=50 --ground-attach-delay=5 --num-tn-gnbs=4 --num-ntn-gnbs=3 --max-ues-tn=20 --max-ues-ntn=10 --rlc-mode=am --monitored-traffic=udp --monitored-dl-rate-mbps=0.2 --monitored-ul-rate-mbps=0.05 --monitored-packet-size=1000 --tn-tx-power-dbm=46 --xhaul-tx-power-dbm=46 --uav-tx-power-dbm=37 --ue-tx-power-dbm=23 --init-min-rsrp=-110 --handover-min-rsrp-dbm=-110 --enable-harq-retx=0 --pdcp-discard-timer-ms=1000 --enable-flow-monitor=1 --enable-rsrp-trace=1 --enable-position-trace=1 --enable-handover-trace=1 --enable-handover-failure-trace=1 --enable-decision-csv=1 --enable-oran-info-log=1 --sat-backhaul-scenario=NTN-Suburban --enable-sat-backhaul-monitor=1 --enable-uav-switching-xapp=1 --uav-control-start=5 --uav-control-period=2 --uav-underserved-rsrp-thresh-dbm=-110 --ue-area-half-w-m=10000 --ue-area-half-h-m=5000 --uav-initial-area-half-w-m=3000 --uav-initial-area-half-h-m=1500 --uav-area-half-w-m=10000 --uav-area-half-h-m=5000 --uav-mission-target-scale=1 --uav-speed-mps=25 --xhaul-pathloss-exponent=5.6 --xhaul-reference-distance-m=100 --xhaul-healthy-rsrp-dbm=-100 --xhaul-switch-to-sat-ttt-s=5 --xhaul-switch-to-tn-ttt-s=5 --enable-xhaul-channel-variation=1 --xhaul-shadowing-stddev-db=6 --xhaul-fading-stddev-db=4 --channel-update-ms=100 --channel-condition-update-ms=200"
```

The TN-only scenario is the clean reference and does not include artificial degradation. The healthy TN + UAV scenario adds aerial access cells under healthy terrestrial donor backhaul, so it tests whether UAVs help when the TN layer has limited capacity for the 100-UE load. The satellite-assisted scenario uses a natural mission movement period from 5 s onward. During this mission period, UEs are distributed over a wider emergency/service area and UAVs move directly toward underserved UE clusters, which can improve UAV access coverage while naturally increasing the UAV-to-TN donor distance. No fixed time-window RSRP penalty is applied:
...
| 5-120 s | UAVs move toward underserved UE clusters; donor RSRP is determined by donor distance, log-distance path loss, and channel variation | If donor RSRP falls below `-100 dBm` for at least 5 s, no-satellite UAV backhaul becomes unavailable; satellite run should switch to fallback when satellite SNR is healthy |
| Recovery periods | If UAV movement or channel variation improves donor RSRP above `-100 dBm` | UAV can return toward `TN_DIRECT`; otherwise fallback/no-backhaul behavior continues |
...
The TN-only case is intentionally not degraded; it establishes the normal terrestrial service level. The TN+UAV case then adds three UAV/NTN cells under healthy terrestrial donor backhaul to test whether aerial cells relieve the 100-UE load. The no-satellite and satellite-assisted distance-loss cases introduce a mission movement period from 5 s onward. During this period, UEs are distributed over a wider emergency/service area and UAVs move directly toward underserved UE clusters within the same mission area. The goal is to keep UAV-to-UE access useful while allowing the UAV-to-TN donor path to become unavailable naturally due to donor distance, RSRP loss, and urban channel variation. The selected donor RSRP and urban shadowing/fading variation determine whether the UAV remains on TN donor backhaul, is blocked without satellite, or switches to satellite fallback.
...
|---|---|---|---|
| Free-space path loss | `fspl` | `gamma = 2.0`, no channel variation | Best-case LOS donor backhaul; TN_DIRECT should dominate and satellite fallback should be rare. |
| Log-distance path loss | `logdist` | `gamma = 5.6`, no channel variation | Stressed obstructed/NLOS-like distance loss; donor RSRP can fall as the UAV moves away from TN donors. |
| Log-distance plus shadowing/fading | `logdist-fading` | `gamma = 5.6`, 6 dB shadowing, 4 dB fading-loss variation | Most variable donor backhaul; more frequent TN-to-satellite switching is expected. |
...
The input features are extracted from `xhaul-autonomy-trace.csv` and nearby QoS measurements in `qos-vs-time.txt`. The current feature set includes donor RSRP, raw/effective donor state, raw-state persistence age, satellite DL/UL SNR, satellite health, switching-xApp availability, recent PDR, recent delay, and recent throughput. Handover counters from `handover-trace.tr`, `handover-failure-trace.tr`, and `ns3-oran-lm.log` can be added as extended features after the first model comparison.
...
The script currently compares Decision Tree, Random Forest, histogram gradient boosting, XGBoost, and a small neural-network classifier. It reports precision, recall, macro-F1, weighted-F1, confusion matrix, and inference time per sample. The comparison table is saved as:
...
A complete final run should produce `final-flow-report.txt`, a `qos-vs-time.txt` trace reaching approximately 120 s, and `xhaul-autonomy-trace.csv` rows showing the raw/effective donor state, donor RSRP, backhaul mode, and switching-xApp decision throughout the 5-120 s mission period.
