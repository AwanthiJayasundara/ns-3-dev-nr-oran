...
GnbRrcMemberNrGnbCmacSapUser::GnbRrcMemberNrGnbCmacSapUser(NrGnbRrc* rrc,
...
uint16_t
GnbRrcMemberNrGnbCmacSapUser::AllocateTemporaryCellRnti()
{
...
void
GnbRrcMemberNrGnbCmacSapUser::RrcConfigurationUpdateInd(UeConfig params)
{
...
NrUeManager::ConfigureMacPhy()
{
    ...
    // schedule this NrUeManager instance to be deleted if the UE does not give any sign of life
...
TypeId
NrUeManager::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::NrUeManager")
            .SetParent<Object>()
            .AddConstructor<NrUeManager>()
            .AddAttribute("DataRadioBearerMap",
                          "List of UE DataRadioBearerInfo by DRBID.",
                          ObjectMapValue(),
                          MakeObjectMapAccessor(&NrUeManager::m_drbMap),
                          MakeObjectMapChecker<NrDataRadioBearerInfo>())
            .AddAttribute("Srb0",
                          "SignalingRadioBearerInfo for SRB0",
                          PointerValue(),
                          MakePointerAccessor(&NrUeManager::m_srb0),
                          MakePointerChecker<NrSignalingRadioBearerInfo>())
            .AddAttribute("Srb1",
                          "SignalingRadioBearerInfo for SRB1",
                          PointerValue(),
                          MakePointerAccessor(&NrUeManager::m_srb1),
                          MakePointerChecker<NrSignalingRadioBearerInfo>())
            .AddAttribute("C-RNTI",
                          "Cell Radio Network Temporary Identifier",
                          TypeId::ATTR_GET, // read-only attribute
                          UintegerValue(0), // unused, read-only attribute
                          MakeUintegerAccessor(&NrUeManager::m_rnti),
                          MakeUintegerChecker<uint16_t>())
            .AddTraceSource("StateTransition",
                            "fired upon every UE state transition seen by the "
                            "NrUeManager at the gNB RRC",
                            MakeTraceSourceAccessor(&NrUeManager::m_stateTransitionTrace),
                            "ns3::NrUeManager::StateTracedCallback")
            .AddTraceSource("DrbCreated",
                            "trace fired after DRB is created",
                            MakeTraceSourceAccessor(&NrUeManager::m_drbCreatedTrace),
                            "ns3::NrUeManager::ImsiCidRntiLcIdTracedCallback");
    return tid;
}

void
NrUeManager::SetSource(uint16_t sourceCellId, uint16_t sourceX2apId)
{
...
void
NrUeManager::InitialContextSetupRequest()
{
                                  ...
                                  Ipv4Address transportLayerAddress)
{
         ...
         ++itLcOnCcMapping)
    {
        ...
        NrGnbCmacSapProvider::LcInfo lcinfo = itLcOnCcMapping->lc;
...
void
NrUeManager::RecordDataRadioBearersToBeStarted()
{
    NS_LOG_FUNCTION(this << (uint32_t)m_rnti);
    for (auto it = m_drbMap.begin(); it != m_drbMap.end(); ++it)
    {
...
void
NrUeManager::StartDataRadioBearers()
{
    NS_LOG_FUNCTION(this << (uint32_t)m_rnti);
...
void
NrUeManager::RecvIdealUeContextRemoveRequest(uint16_t rnti)
{
    NS_LOG_FUNCTION(this << m_rnti);
...
void
NrUeManager::ScheduleRrcConnectionReconfiguration()
{
    NS_LOG_FUNCTION(this);
    switch (m_state)
    {
...
void
NrUeManager::RecvHandoverRequestAck(NrEpcX2SapUser::HandoverRequestAckParams params)
{
    NS_LOG_FUNCTION(this);
    ...
    // note: the Handover command from the target gNB to the source gNB
    ...
    m_targetX2apId = params.newGnbUeX2apId;
    m_targetCellId = params.targetCellId;

    NrEpcX2SapProvider::SnStatusTransferParams sst;
    sst.oldGnbUeX2apId = params.oldGnbUeX2apId;
    sst.newGnbUeX2apId = params.newGnbUeX2apId;
    ...
    for (auto drbIt = m_drbMap.begin(); drbIt != m_drbMap.end(); ++drbIt)
    {
        ...
        if (drbIt->second->m_rlc->GetObject<NrRlcAm>())
        {
...
NrUeManager::GetRadioResourceConfigForHandoverPreparationInfo()
{
    NS_LOG_FUNCTION(this);
...
NrUeManager::GetErabList()
{
    NS_LOG_FUNCTION(this);
    std::vector<NrEpcX2Sap::ErabToBeSetupItem> ret;
    for (auto it = m_drbMap.begin(); it != m_drbMap.end(); ++it)
    {
...
void
NrUeManager::SendUeContextRelease()
{
    NS_LOG_FUNCTION(this);
    switch (m_state)
    {
    case HANDOVER_PATH_SWITCH:
        NS_LOG_INFO("Send UE CONTEXT RELEASE from target gNB to source gNB");
        NrEpcX2SapProvider::UeContextReleaseParams ueCtxReleaseParams;
        ueCtxReleaseParams.oldGnbUeX2apId = m_sourceX2apId;
        ueCtxReleaseParams.newGnbUeX2apId = m_rnti;
...
void
NrUeManager::RecvHandoverPreparationFailure(uint16_t cellId)
{
    NS_LOG_FUNCTION(this << cellId);
    switch (m_state)
    {
...
void
NrUeManager::RecvSnStatusTransfer(NrEpcX2SapUser::SnStatusTransferParams params)
{
    NS_LOG_FUNCTION(this);
...
void
NrUeManager::RecvHandoverCancel(NrEpcX2SapUser::HandoverCancelParams params)
{
    NS_LOG_FUNCTION(this);
...
void
NrUeManager::SendRrcConnectionRelease()
{
    ...
    NrRrcSap::RrcConnectionRelease msg;
...
void
NrUeManager::CompleteSetupUe(NrGnbRrcSapProvider::CompleteSetupUeParameters params)
{
    NS_LOG_FUNCTION(this);
...
void
NrUeManager::RecvRrcConnectionRequest(NrRrcSap::RrcConnectionRequest msg)
{
    NS_LOG_FUNCTION(this);
    switch (m_state)
    {
...
void
NrUeManager::RecvRrcConnectionSetupCompleted(NrRrcSap::RrcConnectionSetupCompleted msg)
{
    NS_LOG_FUNCTION(this);
    switch (m_state)
    {
    ...
    NrRrcSap::RrcConnectionReconfigurationCompleted msg)
{
    ...
    case CONNECTION_RECONFIGURATION:
        ...
        if (m_needPhyMacConfiguration)
        {
            ...
            for (uint16_t i = 0; i < m_rrc->m_numberOfComponentCarriers; i++)
            {
                ...
                double paDouble = NrRrcSap::ConvertPdschConfigDedicated2Double(
    ...
    case HANDOVER_JOINING: {
        ...
        NrEpcGnbS1SapProvider::PathSwitchRequestParameters params;
        params.rnti = m_rnti;
...
} // end of NrUeManager::RecvMeasurementReport

// methods forwarded from CMAC SAP

void
NrUeManager::CmacUeConfigUpdateInd(NrGnbCmacSapUser::UeConfig cmacParams)
{
    NS_LOG_FUNCTION(this << m_rnti);
...
void
NrUeManager::DoReceivePdcpSdu(NrPdcpSapUser::ReceivePdcpSduParameters params)
{
    NS_LOG_FUNCTION(this);
    if (params.lcid > 2)
    {
...
uint16_t
NrUeManager::GetRnti() const
{
    return m_rnti;
}

uint64_t
NrUeManager::GetImsi() const
{
    return m_imsi;
}
...
void
NrUeManager::SetPdschConfigDedicated(NrRrcSap::PdschConfigDedicated pdschConfigDedicated)
{
    NS_LOG_FUNCTION(this);
...
NrEpcX2Sap::HandoverPreparationFailureParams
NrUeManager::BuildHoPrepFailMsg()
{
    NS_LOG_FUNCTION(this);
    NrEpcX2Sap::HandoverPreparationFailureParams res;
    res.oldGnbUeX2apId = m_sourceX2apId;
...
NrEpcX2Sap::HandoverCancelParams
NrUeManager::BuildHoCancelMsg()
{
    NS_LOG_FUNCTION(this);
    NrEpcX2Sap::HandoverCancelParams res;
    res.oldGnbUeX2apId = m_rnti; // source cell rnti
    res.newGnbUeX2apId = m_targetX2apId;
...
NrUeManager::GetDataRadioBearerInfo(uint8_t drbid)
{
...
void
NrUeManager::RemoveDataRadioBearerInfo(uint8_t drbid)
{
    NS_LOG_FUNCTION(this << (uint32_t)drbid);
    auto it = m_drbMap.find(drbid);
    NS_ASSERT_MSG(it != m_drbMap.end(),
...
NrRrcSap::RrcConnectionReconfiguration
NrUeManager::BuildRrcConnectionReconfiguration()
{
    NS_LOG_FUNCTION(this);
    NrRrcSap::RrcConnectionReconfiguration msg{};
...
uint8_t
NrUeManager::GetNewRrcTransactionIdentifier()
{
    NS_LOG_FUNCTION(this);
...
void
NrUeManager::SwitchToState(State newState)
{
    NS_LOG_FUNCTION(this << ToString(newState));
    State oldState = m_state;
    m_state = newState;
    NS_LOG_INFO(this << " IMSI " << m_imsi << " RNTI " << m_rnti << " NrUeManager "
...
{
    NS_LOG_FUNCTION(this);
    NrRrcSap::NonCriticalExtensionConfiguration ncec;
    ...
    for (auto& it : m_rrc->m_componentCarrierPhyConf)
    {
        ...
        component.radioResourceConfigDedicatedSCell.physicalConfigDedicatedSCell
            .soundingRsUlConfigDedicated.type = NrRrcSap::SoundingRsUlConfigDedicated::SETUP;
        component.radioResourceConfigDedicatedSCell.physicalConfigDedicatedSCell
      ...
      m_carriersConfigured(false)
{
    ...
    m_handoverManagementSapUser = new MemberNrHandoverManagementSapUser<NrGnbRrc>(this);
    m_anrSapUser = new MemberNrAnrSapUser<NrGnbRrc>(this);
    m_rrcSapProvider = new MemberNrGnbRrcSapProvider<NrGnbRrc>(this);
    m_x2SapUser = new NrEpcX2SpecificEpcX2SapUser<NrGnbRrc>(this);
...
void
NrGnbRrc::ConfigureCarriers(std::map<uint8_t, Ptr<BandwidthPartGnb>> ccPhyConf)
{
    ...
    {
        m_cphySapUser.push_back(new MemberNrGnbCphySapUser<NrGnbRrc>(this));
        m_cmacSapUser.push_back(new GnbRrcMemberNrGnbCmacSapUser(this, i));
    }
    ...
    static TypeId tid =
        TypeId("ns3::NrGnbRrc")
            ...
            .AddAttribute("UeMap",
                          ...
                          MakeObjectMapChecker<NrUeManager>())
            ...
            .AddTraceSource("ConnectionReconfiguration",
                            ...
                            MakeTraceSourceAccessor(&NrGnbRrc::m_connectionReconfigurationTrace),
                            "ns3::NrGnbRrc::ConnectionHandoverTracedCallback")
            ...
            .AddTraceSource("NotifyConnectionRelease",
                            ...
                            MakeTraceSourceAccessor(&NrGnbRrc::m_connectionReleaseTrace),
                            "ns3::NrGnbRrc::ConnectionHandoverTracedCallback")
            ...
            .AddTraceSource(
                ...
                "preamble at gNB for UE to handover due to max count reached",
...
void
NrGnbRrc::SetEpcX2SapProvider(NrEpcX2SapProvider* s)
{
...
void
NrGnbRrc::SetNrGnbCmacSapProvider(NrGnbCmacSapProvider* s)
{
    NS_LOG_FUNCTION(this << s);
...
void
NrGnbRrc::SetNrGnbCmacSapProvider(NrGnbCmacSapProvider* s, uint8_t pos)
{
    NS_LOG_FUNCTION(this << s);
...
NrGnbCmacSapUser*
NrGnbRrc::GetNrGnbCmacSapUser(uint8_t pos)
{
    NS_LOG_FUNCTION(this);
...
void
NrGnbRrc::SetNrHandoverManagementSapProvider(NrHandoverManagementSapProvider* s)
{
    NS_LOG_FUNCTION(this << s);
...
NrGnbRrc::GetNrHandoverManagementSapUser()
{
    NS_LOG_FUNCTION(this);
...
void
NrGnbRrc::SetNrCcmRrcSapProvider(NrCcmRrcSapProvider* s)
{
    NS_LOG_FUNCTION(this << s);
...
void
NrGnbRrc::SetNrAnrSapProvider(NrAnrSapProvider* s)
{
    NS_LOG_FUNCTION(this << s);
    m_anrSapProvider = s;
}
...
void
NrGnbRrc::SetS1SapProvider(NrEpcGnbS1SapProvider* s)
{
    m_s1SapProvider = s;
}
...
std::vector<uint8_t>
NrGnbRrc::AddUeMeasReportConfig(NrRrcSap::ReportConfigEutra config)
{
    NS_LOG_FUNCTION(this);
...
NrGnbRrc::ConnectionRequestTimeout(uint16_t rnti)
{
    ...
    NS_ASSERT_MSG(GetUeManager(rnti)->GetState() == NrUeManager::INITIAL_RANDOM_ACCESS,
...
{
    NS_LOG_FUNCTION(this << rnti);
    NS_ASSERT_MSG(GetUeManager(rnti)->GetState() == NrUeManager::CONNECTION_SETUP,
...
{
    NS_LOG_FUNCTION(this << rnti);
    NS_ASSERT_MSG(GetUeManager(rnti)->GetState() == NrUeManager::CONNECTION_REJECTED,
...
void
NrGnbRrc::DoRecvRrcConnectionRequest(uint16_t rnti, NrRrcSap::RrcConnectionRequest msg)
{
    NS_LOG_FUNCTION(this << rnti);
...
NrGnbRrc::DoRecvIdealUeContextRemoveRequest(uint16_t rnti)
{
    ...
    if (HasUeManager(rnti))
    {
        ...
        if (ueManager->GetState() == NrUeManager::HANDOVER_JOINING)
        {
             ...
             * re-attempts is reached the UE request the target gNB to deletes its
             * context. Upon which, the target gNB sends handover preparation
...
NrGnbRrc::DoRecvHandoverRequest(NrEpcX2SapUser::HandoverRequestParams req)
{
    ...
    NS_LOG_LOGIC("oldGnbUeX2apId = " << req.oldGnbUeX2apId);
    ...
    NrEpcX2SapProvider::HandoverRequestAckParams ackParams;
    ackParams.oldGnbUeX2apId = req.oldGnbUeX2apId;
    ackParams.newGnbUeX2apId = rnti;
    ...
    NrGnbCmacSapProvider::RachConfig rc = m_cmacSapProvider.at(componentCarrierId)->GetRachConfig();
    ...
    NS_LOG_LOGIC("oldGnbUeX2apId = " << ackParams.oldGnbUeX2apId);
    NS_LOG_LOGIC("newGnbUeX2apId = " << ackParams.newGnbUeX2apId);
...
void
NrGnbRrc::DoRecvHandoverRequestAck(NrEpcX2SapUser::HandoverRequestAckParams params)
{
    NS_LOG_FUNCTION(this);
...
void
NrGnbRrc::DoRecvHandoverPreparationFailure(NrEpcX2SapUser::HandoverPreparationFailureParams params)
{
    NS_LOG_FUNCTION(this);
...
NrGnbRrc::DoRecvSnStatusTransfer(NrEpcX2SapUser::SnStatusTransferParams params)
{
    ...
    uint16_t rnti = params.newGnbUeX2apId;
...
void
NrGnbRrc::DoRecvLoadInformation(NrEpcX2SapUser::LoadInformationParams params)
{
    NS_LOG_FUNCTION(this);
...
void
NrGnbRrc::DoRecvResourceStatusUpdate(NrEpcX2SapUser::ResourceStatusUpdateParams params)
{
    NS_LOG_FUNCTION(this);
...
void
NrGnbRrc::DoRecvUeData(NrEpcX2SapUser::UeDataParams params)
{
    NS_LOG_FUNCTION(this);
...
void
NrGnbRrc::DoRecvHandoverCancel(NrEpcX2SapUser::HandoverCancelParams params)
{
    NS_LOG_FUNCTION(this);
...
NrGnbRrc::DoAllocateTemporaryCellRnti(uint8_t componentCarrierId)
{
    ...
    return AddUe(NrUeManager::INITIAL_RANDOM_ACCESS, componentCarrierId);
}
...
std::vector<uint8_t>
NrGnbRrc::DoAddUeMeasReportConfigForHandover(NrRrcSap::ReportConfigEutra reportConfig)
{
    NS_LOG_FUNCTION(this);
...
uint8_t
NrGnbRrc::DoAddUeMeasReportConfigForComponentCarrier(NrRrcSap::ReportConfigEutra reportConfig)
{
    NS_LOG_FUNCTION(this);
...
uint8_t
NrGnbRrc::DoAddUeMeasReportConfigForAnr(NrRrcSap::ReportConfigEutra reportConfig)
{
    NS_LOG_FUNCTION(this);
    uint8_t measId = AddUeMeasReportConfig(reportConfig).at(0);
...
void
NrGnbRrc::DoSendLoadInformation(NrEpcX2Sap::LoadInformationParams params)
{
    NS_LOG_FUNCTION(this);
...
uint16_t
NrGnbRrc::AddUe(NrUeManager::State state, uint8_t componentCarrierId)
{
    NS_LOG_FUNCTION(this);
    bool found = false;
    uint16_t rnti;
